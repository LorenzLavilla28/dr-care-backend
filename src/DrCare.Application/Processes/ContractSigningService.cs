...
public sealed class ContractSigningService(IProcessRepository processes, ILeadRepository leads, IPrivateObjectStorage storage, IDocumentPdfRenderer renderer, ICurrentUser currentUser, INotificationService notifications) : IContractSigningService
{
    ...
    public async Task<SigningRequestResponse> CreateAsync(Guid leadId, CreateSigningRequest request, CancellationToken ct)
    {
        ...
        foreach (var item in existing.Where(x => x.SignerRole.Equals(request.SignerRole, StringComparison.OrdinalIgnoreCase) && x.Status is SigningRequestStatus.Pending or SigningRequestStatus.Viewed)) item.Void();
    ...
    public async Task<SigningRequestResponse> VoidAsync(Guid leadId, Guid requestId, CancellationToken ct)
    {
        ...
        item.Void(); await processes.SaveChangesAsync(ct); return ToResponse(item);
    }
    ...
    public async Task<PublicSigningResponse> GetPublicAsync(string token, CancellationToken ct)
    {
        ...
        var documentUrl = string.IsNullOrWhiteSpace(key) ? null : await storage.CreateDownloadUrlAsync(key, DateTimeOffset.UtcNow.AddMinutes(10), ct);
    ...
    public async Task<PublicSigningResponse> SignPublicAsync(string token, SignContractRequest request, string? ipAddress, string? userAgent, CancellationToken ct)
    {
        ...
        if (contract.Status == ContractStatus.Signed)
        {
            ...
            await notifications.NotifyUserAsync(item.OrganizationId, lead.AssignedAgentId, "contract-signed", "Contract signed", $"Both parties signed the contract for {lead.FullName}. Begin the pre-launch checklist.", ct);
        }
    ...
    {
        var item = await GetActiveByTokenAsync(token, ct); try { item.Decline(request.Reason); } catch (DomainRuleException ex) { throw new ConflictException(ex.Message); } await processes.SaveChangesAsync(ct);
    }
    ...
    private static SigningRequestResponse ToResponse(ContractSigningRequest x, string? url = null) => new(x.Id, x.LeadId, x.ContractId, x.SignerRole, x.SignerName, x.SignerEmail, x.Status.ToString(), x.ExpiresAt, x.SignedAt, url);
}
